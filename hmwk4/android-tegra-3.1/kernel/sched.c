...
/* Weighted round robin fields in runqueue */
struct wrr_rq {

	unsigned long wrr_nr_running; /* # of runnable tasks on this q*/
	struct list_head wrr_list; /* list of runnable tasks */

#ifdef CONFIG_SMP
	int weight_sum; /* Weight of all tasks on this wrr_rq */
	struct plist_head movable_tasks; /* List of tasks eligible to be
					   * moved ordered by weight */
#endif
};
...
void sched_fork(struct task_struct *p)
{
	...
	if (!reset)
		p->wrr_weight = current->wrr_weight;
...
__setscheduler(struct rq *rq, struct task_struct *p, int policy, int prio)
{
	...
	if (rt_prio(p->prio))
		...
		p->sched_class = &wrr_sched_class; /* CHANGED HERE */
	else
		p->sched_class = &fair_sched_class;
	set_load_weight(p);
}
...
static void init_wrr_rq(struct wrr_rq *wrr_rq)
{
	...
	INIT_LIST_HEAD(&wrr_rq->wrr_list);
...
SYSCALL_DEFINE2(sched_setweight, pid_t, pid, int, weight)
{
	...
	/* find_process_by_pid will return current if pid is zero */
	p = find_process_by_pid(pid);
	if (p == NULL) {
		rcu_read_unlock();
		return -ESRCH;
	...
	/* user can only change weight if they're root or owner */
	...
	if (current_uid() != 0 && current_euid() != 0) {
		...
		/* If not root user you can only decrease weight */
		if (p->wrr_weight < weight) {
			rcu_read_unlock();
			return -EACCES;
		}
	}
